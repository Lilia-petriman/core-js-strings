...
function formatTime(minutes, seconds) {
  ...
  const sec = seconds.toString().padStart(3, ':0');
...
function isPalindrome(str) {
  const regex = /[\W_]/g;
  const lowerStr = str.toLowerCase().replace(regex, '');
...
function findLongestWord(sentence) {
  const wordSent = sentence.split(' ');
  const orderedArr = wordSent.sort((a, b) => b.length - a.length);
  return orderedArr[0];
}
...
function extractNameFromTemplate(value) {
  const byIndex = value.indexOf(',');
  let name = value.substring(byIndex + 1).trim();
  name = name.slice(0, -1);
  return name;
}
...
function extractEmails(str) {
  const emailAll = str.split(';').map((email) => email.trim());
  return emailAll;
}
...
function encodeToRot13(str) {
  ...
  const ornInput = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  return str.replace(/[A-Z]/gi, (c) => {
    const index = ornInput.indexOf(c);
    return rot13Output[index];
  });
}
